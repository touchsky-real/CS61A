...
def sus_points(score):
    ...
    "*** YOUR CODE HERE ***"
    nextscore = score
    if not (num_factors(score) ==3 or num_factors(score) ==4):
        return score
    else:
        while not is_prime(nextscore):
            nextscore += 1
        return nextscore
    # END PROBLEM 4
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while i < goal:
        ...
        while j < goal:
            if strategy(i, j) != num:
                is_always_roll = False
                break
            j += 1
        i += 1
    return is_always_roll
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    while i <=10:
        current_score=averaged_dice(i, dice)
