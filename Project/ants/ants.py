...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        self.damage *= 2 # set a new instance attribute with same name
        self.isdoubled = True
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        placeindex = 0
        place = self.place
        while not place.is_hive and placeindex <= self.upper_bound:
            if placeindex < self.lower_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # Ant.reduce_health(self,amount)  # alternative way
    ...
    def action(self, gamestate):
        if self.cooldown:
            ...
            if not self.place.bees:
                ...
                targetBee = random_bee(self.place.bees)
                targetBee.reduce_health(targetBee.health)
    ...
    def action(self, gamestate):
        ...
        while place.exit is not None:
            if not place.ant:
                place = place.exit
                continue
            elif place.ant.is_container and place.ant.ant_contained:
                if not place.ant.isdoubled:
                    place.ant.double()
                if not place.ant.ant_contained.isdoubled:
