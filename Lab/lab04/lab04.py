def divide(quotients, divisors):
    ...
    """
    return {q: [d for d in divisors if not d%q] for q in quotients}
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    targetCity = make_city("target", lat, lon)
    closerCity = city_a
    if distance(targetCity, city_b) < distance(targetCity, closerCity):
        closerCity = city_b
    return get_name(closerCity)
