def shuffle(s):
    ...
    "*** YOUR CODE HERE ***"
    # shuffledList = list(s[:len(s) // 2])
    # for i in range(len(s)-1,(len(s) // 2)-1,-1):
    #     shuffledList.insert(i-len(s) // 2+1,s[i])
    mid = len(s) // 2
...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    lMass = total_mass(end(left(m)))
    rMass = total_mass(end(right(m)))
    equaltorque = lMass*length(left(m)) == rMass*length(right(m))
    ...
    if is_planet(end(left(m))) and is_planet(end(right(m))):
        ...
        if is_mobile(end(left(m))):
            mobiles.append(end(left(m)))
        if is_mobile(end(right(m))):
            mobiles.append(end(right(m)))
        return all([balanced(mobile)for mobile in mobiles]) and equaltorque
